...
public class Day7Part2 extends Day<Integer> {
    ...
    Map<String,Integer> sizeByDirectory = new HashMap<>();

    @Override
    public Integer process() {
        readTerminal(allLines());
        calculateSizes(root);
        int emptySpaceNeeded = 30000000 - (70000000 - sizeByDirectory.get("/"));
        return sizeByDirectory.values().stream()
                .filter(directorySize -> directorySize >= emptySpaceNeeded)
    ...
    private int calculateSizes(Directory current) {
        ...
        sizeByDirectory.put(current.getName(), totalFilesSize);
